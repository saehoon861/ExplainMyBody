...
def save_inbody():
    """수정된 인바디 데이터를 Pydantic으로 검증하고 저장 준비"""
    try:
        ...
        try:
            ...
            # 여기서 실제로 DB 저장 로직이 들어갑니다.
            # 예: db.save(validated_data.dict())
            
            print("=" * 30)
            print("✅ Pydantic 검증 완료")
            print(f"추출된 데이터: {validated_data.기본정보}")
            print("=" * 30)
            
            return jsonify({
                'success': True,
                'message': '인바디 데이터가 정상적으로 검증되었습니다.',
                'validated_data': validated_data.dict()
            })
        except Exception as ve:
            return jsonify({'error': f'데이터 검증 실패: {str(ve)}'}), 422
