"""
LLM 호출 성능 테스트 - Sync 순차 방식
4개의 LLM Call을 하나씩 순차적으로 실행
"""
...
def test_sync_sequential():
    ...
    for key, (system_prompt, user_prompt) in prompts.items():
        ...
        plan_results[key] = response

        call_end = time.time()
        call_duration = call_end - call_start
        individual_times[key] = call_duration

        print(f"완료 ({call_duration:.3f}초)")
    ...
    # 결과 출력
    print(f"\n🤖 모든 LLM 호출 완료: {(llm_end_time - llm_start_time):.3f}초")
    ...
    print(f"   - LLM 호출 (순차): {(llm_end_time - llm_start_time):.3f}초")
    print(f"   - 후처리: {(end_time - llm_end_time):.3f}초")

    # 개별 LLM 호출 시간 출력
    print("\n⏱️  개별 LLM 호출 시간:")
    for key, duration in individual_times.items():
        print(f"   - {key}: {duration:.3f}초")
...
if __name__ == "__main__":
    total_time = test_sync_sequential()
    print(f"✅ Sync 순차 방식 총 소요 시간: {total_time:.3f}초")
