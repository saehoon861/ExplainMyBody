...
from .prompt_generator_rag import create_inbody_analysis_prompt_with_rag
...
def create_analysis_agent_with_rag(llm_client, use_rag: bool = True):
    ...
    def generate_initial_analysis(state: AnalysisStateRAG) -> dict:
        """Node 1: RAG 검색 + 최초 분석 결과 생성 및 임베딩"""
        print("--- LLM1 (RAG): 최초 분석 생성 ---")
        ...
        # 3. 프롬프트 생성 및 LLM 호출 (RAG 컨텍스트 포함)
        system_prompt, user_prompt = create_inbody_analysis_prompt_with_rag(
            measurements,
