...
"""
    ...
    user_prompt_parts.append(f"- 주차: 1주차")
    ...
    user_prompt_parts.append("\n### 주요 수치")
    user_prompt_parts.append(f"- 성별/나이: {measurements.기본정보.성별}, {measurements.기본정보.연령}세")
    user_prompt_parts.append(f"- 신장/체중: {measurements.기본정보.신장}cm, {measurements.체중관리.체중}kg")
    user_prompt_parts.append(f"- BMI: {measurements.비만분석.BMI}")
    user_prompt_parts.append(f"- 체지방률: {measurements.비만분석.체지방률}%")
    user_prompt_parts.append(f"- 골격근량: {measurements.체중관리.골격근량} kg")
    if measurements.연구항목.기초대사량:
    ...
    if hasattr(goal_input, 'body_type1') and goal_input.body_type1:
         user_prompt_parts.append(f"- 체형: {goal_input.body_type1} / {goal_input.body_type2}")

    user_prompt_parts.append("\n## 사용자 목표")
    user_prompt_parts.append(f"- 유형: {goal_input.user_goal_type}")
    user_prompt_parts.append(f"- 상세 내용: {goal_input.user_goal_description}")

    user_prompt_parts.append("\n## 기타 선호도 및 특이사항")
    user_prompt_parts.append("(별도의 구조화된 선호도 데이터가 없으므로, 사용자 목표 상세 내용이나 일반적인 건강 가이드라인을 따르세요.)")
    ...
    user_prompt_parts.append("- 사용자 제약사항 및 선호도(목표 상세 내용에 있다면) 반드시 준수")
