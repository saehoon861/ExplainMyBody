"""
주간 계획 라우터 (RAG 버전)
/api/weekly-plans-rag/*

RAG 기능이 추가된 주간 계획 엔드포인트
- 논문 DB 기반 과학적 근거 제공 --> 수정된부분은 weekly_plan_service_rag 부분
"""
...
from services.llm.llm_rag.weekly_plan_service_rag import WeeklyPlanServiceRAG
from typing import List
from datetime import date

router = APIRouter()
weekly_plan_service_rag = WeeklyPlanServiceRAG()


@router.post("/generate", response_model=WeeklyPlanResponse, status_code=201)
async def generate_weekly_plan_with_rag(
    user_id: int,
    ...
    - **RAG**: 논문 DB에서 관련 운동/식단 연구 자동 검색 및 계획에 반영

    Returns:
        생성된 주간 계획 (마크다운 형식의 content 포함)
    """
    try:
        new_plan = await weekly_plan_service_rag.generate_plan(db, user_id, request)
        return new_plan
...
async def chat_with_plan_rag(
    plan_id: int,
    chat_request: WeeklyPlanChatRequest,
    db: Session = Depends(get_db)
):
    """
    생성된 주간 계획에 대한 질의응답 및 수정 요청 (RAG)
    ...
    try:
        ...
        response = await weekly_plan_service_rag.chat_with_plan(
            plan_id=plan_id,
