...
class KCIAPICollector:
    ...
    def __init__(self, api_key: str):
        ...
        # KCI Open API 엔드포인트 (공공데이터포털)
        self.base_url = "https://apis.data.go.kr/B552540/KCIOpenApi/artiInfo"
    ...
    def search_papers(
        ...
        for page in range(1, total_pages + 1):
            try:
                # API 요청 파라미터 (공공데이터포털 형식)
                params = {
                    'serviceKey': self.api_key,  # 공공데이터포털 인증키
                    'numOfRows': min(page_size, max_results - len(papers)),  # 한 페이지 결과 수
                    'pageNo': page,  # 페이지 번호
                    'type': 'xml',  # 응답 형식
                    'title': query,  # 제목 검색
                }
                ...
                # 총 결과 수 확인
                total_elem = root.find('.//body/totalCount')
